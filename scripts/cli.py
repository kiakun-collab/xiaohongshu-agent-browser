...
def _output(data: dict, exit_code: int = 0) -> None:
    """输出 JSON 并退出。"""
    if exit_code != 0:
        _capture_failure_artifacts(str(data.get("error", "")))

    payload = dict(data)
    if _EXECUTION_CONTEXT.get("run_id"):
        payload.setdefault("run_id", _EXECUTION_CONTEXT["run_id"])
        payload.setdefault("artifacts_dir", _EXECUTION_CONTEXT["run_dir"])
        if _EXECUTION_CONTEXT.get("failure_artifacts"):
            payload.setdefault("failure_artifacts", _EXECUTION_CONTEXT["failure_artifacts"])

    _finalize_execution(payload, exit_code)
    print(json.dumps(payload, ensure_ascii=False, indent=2))
    sys.exit(exit_code)


def _resolved_account(args: argparse.Namespace) -> str:
    """解析当前命令实际使用的账号名。"""
    from account_manager import resolve_account_name

    return resolve_account_name(getattr(args, "account", ""))


def _profile_dir(args: argparse.Namespace) -> str:
    """解析当前命令对应的 Chrome Profile 目录。"""
    from account_manager import get_profile_dir

    return get_profile_dir(getattr(args, "account", ""))


def _save_active_page(
    ...
    *,
    ...
    """保存当前分步流程对应的页面。"""
    from runtime_state import save_active_page

    save_active_page(args.port, _resolved_account(args), target_id, flow=flow, stage=stage)


def _clear_active_page(args: argparse.Namespace) -> None:
    """清除当前分步流程对应的页面。"""
    from runtime_state import clear_active_page

    clear_active_page(args.port, _resolved_account(args))
...
def _begin_execution(args: argparse.Namespace) -> None:
    from operation_logger import start_command

    metadata = start_command(args.command, _resolved_account(args), _serializable_args(args))
    _EXECUTION_CONTEXT.update(
        {
            ...
            "account": _resolved_account(args),
            "page": None,
            "finalized": False,
            "failure_artifacts": {},
        }
...
def _bind_page(page: Any) -> None:
    _EXECUTION_CONTEXT["page"] = page


def _capture_failure_artifacts(reason: str = "") -> None:
    if (
        not _EXECUTION_CONTEXT.get("run_id")
        or _EXECUTION_CONTEXT.get("failure_artifacts")
        or _EXECUTION_CONTEXT.get("page") is None
    ):
        return

    from operation_logger import capture_failure_artifacts

    artifacts = capture_failure_artifacts(
        _EXECUTION_CONTEXT["page"],
        _EXECUTION_CONTEXT["run_id"],
        reason=reason,
    )
    if artifacts:
        _EXECUTION_CONTEXT["failure_artifacts"] = artifacts


def _finalize_execution(result: dict[str, Any], exit_code: int) -> None:
    if not _EXECUTION_CONTEXT.get("run_id") or _EXECUTION_CONTEXT.get("finalized"):
        return

    from operation_logger import finish_command

    finish_command(
        _EXECUTION_CONTEXT["run_id"],
        exit_code=exit_code,
        result=result,
        failure_artifacts=_EXECUTION_CONTEXT.get("failure_artifacts"),
    )
    _EXECUTION_CONTEXT["finalized"] = True


def _finalize_browser(browser: Any, page: Any | None = None, *, keep_page_open: bool = False) -> None:
    if sys.exc_info()[0] is not None:
        _capture_failure_artifacts(str(sys.exc_info()[1]))
...
def _connect(args: argparse.Namespace):
    ...
    if not ensure_chrome(port=args.port, user_data_dir=_profile_dir(args)):
        _output(
...
def _connect_existing(args: argparse.Namespace):
    ...
    from chrome_launcher import ensure_chrome
    from runtime_state import clear_active_page, load_active_page
    from xhs.cdp import Browser
    ...
    browser = Browser(host=args.host, port=args.port)
    browser.connect()
    state = load_active_page(args.port, account)
    if not state or not state.get("target_id"):
        browser.close()
        _output(
            {"success": False, "error": "未找到流程页面，请先执行前置步骤"},
            exit_code=2,
        )

    page = browser.get_existing_page(target_id=state["target_id"])
    if not page:
        browser.close()
        clear_active_page(args.port, account)
        _output(
            {"success": False, "error": "流程页面已失效或被关闭，请重新执行前置步骤"},
            exit_code=2,
        )
    _bind_page(page)
    return browser, page
...
def _headless_fallback(args: argparse.Namespace) -> None:
    ...
    restart_chrome(port=args.port, headless=False, user_data_dir=_profile_dir(args))
    _output(
        {
            "success": False,
            "error": "未登录",
...
def cmd_check_login(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        _finalize_browser(browser, page)
...
def cmd_send_code(args: argparse.Namespace) -> None:
    ...
    for attempt in range(2):
        ...
        try:
            ...
            _save_active_page(args, page.target_id, flow="phone-login", stage="awaiting-code")
            result = {
                "status": "code_sent",
            ...
            if attempt == 0:
                ...
                restart_chrome(port=args.port, user_data_dir=_profile_dir(args))
                continue
...
def cmd_fill_publish(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        _save_active_page(args, page.target_id, flow="image-publish", stage="awaiting-publish")
        _output(
            {
                "success": True,
                "title": title,
        ...
        # 不关闭页面，让用户在浏览器中预览
        _finalize_browser(browser, page, keep_page_open=True)
...
def cmd_fill_publish_video(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        _save_active_page(args, page.target_id, flow="video-publish", stage="awaiting-publish")
        _output(
            {
                "success": True,
                "title": title,
...
def cmd_long_article(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        _save_active_page(args, page.target_id, flow="long-article", stage="awaiting-template")
        _output(
            {
                "success": True,
...
def cmd_select_template(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        if selected:
            _save_active_page(args, page.target_id, flow="long-article", stage="template-selected")
...
def cmd_next_step(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        _save_active_page(args, page.target_id, flow="long-article", stage="awaiting-publish")
