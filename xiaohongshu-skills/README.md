...
**固定运营流程：**
> "先帮我调研露营赛道最近热门选题，再给我写一版草稿，等我确认后再发"
...
1. `check-login`
2. `search-feeds --keyword "<赛道关键词>"`
3. 从搜索结果中挑选高互动笔记
4. 对 3 到 5 篇笔记执行 `get-feed-detail`
5. 输出：
   - 高频标题词
   - 常见内容结构
   - 评论区痛点
...
1. 先做选题分析，或使用你直接给定的标题/正文方向
2. 由 Agent 生成标题、正文、标签建议
3. 使用 `fill-publish` 或 `fill-publish-video` 只填写发布页，不直接发布
4. 你在浏览器中检查内容
5. 再由你明确决定：
   - 执行 `click-publish`
   - 或执行 `save-draft`
...
从当前版本开始，每次 CLI 执行都会写入运行日志，并返回：

- `run_id`
- `artifacts_dir`
- 失败时的 `failure_artifacts`

默认目录：

```bash
~/.xhs/logs/<YYYY-MM-DD>/<run_id>/
```

其中通常包含：

- `run.json`：本次执行的结果与元数据
- `failure.png`：失败时的浏览器截图
- `failure-context.json`：失败时的页面 URL、标题与原因
