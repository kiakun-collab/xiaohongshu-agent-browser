...
class Page:
    ...
    def input_content_editable(self, selector: str, text: str) -> None:
        ...
        time.sleep(0.1)
        # 2. 全选清空（macOS 使用 Command+A，其它平台使用 Ctrl+A）
        select_all_modifier = _select_all_modifier_value()
        self._send_session(
            "Input.dispatchKeyEvent",
            {"type": "keyDown", "key": "a", "code": "KeyA", "modifiers": select_all_modifier},
        )
        self._send_session(
            "Input.dispatchKeyEvent",
    ...
    def capture_screenshot(self, output_path: str) -> str:
        """保存当前页面截图。"""
        result = self._send_session("Page.captureScreenshot", {"format": "png"})
        data = result.get("data")
        if not data:
            raise CDPError("页面截图失败：未返回图片数据")
...
class Browser:
    ...
    def new_page(self, url: str = "about:blank") -> Page:
        ...
        return self._configure_page(Page(self._cdp, target_id, session_id), emulate=True)

    def _configure_page(self, page: Page, *, emulate: bool) -> Page:
        """初始化页面会话。"""
        page.inject_stealth()
        if emulate:
            page._send_session(
    ...
    def attach_to_target(self, target_id: str) -> Page | None:
        """附着到指定 target。"""
        if not self._cdp:
            self.connect()
        assert self._cdp is not None

        if not any(target.get("id") == target_id for target in self.list_page_targets()):
            return None
    ...
    def get_existing_page(self, target_id: str | None = None) -> Page | None:
        ...
        for target in self.list_page_targets():
            if target.get("url") != "about:blank":
                return self.attach_to_target(target["id"])
        return None
