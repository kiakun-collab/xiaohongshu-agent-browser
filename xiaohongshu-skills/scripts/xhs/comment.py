...
def _wait_for_comment_submission(page: Page, before_count: int | None, timeout_seconds: float = 6.0) -> None:
    """等待评论/回复提交结果。"""
    import time
    ...
    while time.monotonic() < deadline:
        if before_count is not None:
            try:
                if _get_comment_count(page) > before_count:
                    return
            except Exception:
                pass
        ...
        input_empty = page.evaluate(
            f"""
            (() => {{
                const el = document.querySelector({COMMENT_INPUT_FIELD!r});
                return !el || !el.textContent || el.textContent.trim().length === 0;
            }})()
            """
        )
        if input_empty is True:
            return
        time.sleep(0.5)

    raise RuntimeError("提交后未观察到评论区变化，可能发送失败")
...
def post_comment(page: Page, feed_id: str, xsec_token: str, content: str) -> None:
    ...
    try:
        before_count = _get_comment_count(page)
    except Exception:
        before_count = None
    ...
    _wait_for_comment_submission(page, before_count)
