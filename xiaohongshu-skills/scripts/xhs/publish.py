...
def click_publish_button(page: Page) -> None:
    ...
    _wait_for_publish_transition(page, initial_url)
    logger.info("发布完成")
...
def _wait_for_publish_transition(page: Page, initial_url: str | None, timeout: float = 30.0) -> None:
    """等待发布动作进入下一状态，避免误报成功。"""
    ...
    while time.monotonic() < deadline:
        state = page.evaluate(
            f"""
            (() => {{
                const btn = document.querySelector({json.dumps(PUBLISH_BUTTON)});
                const bodyText = document.body ? document.body.innerText : '';
                return {{
                    url: window.location.href,
                    buttonPresent: !!btn,
                    buttonDisabled: !!btn && (btn.disabled || btn.classList.contains('disabled')),
                    bodyText,
                }};
            }})()
            """
        ...
        button_present = bool(state.get("buttonPresent"))
        button_disabled = bool(state.get("buttonDisabled"))

        if initial_url and current_url and current_url != initial_url:
            return
        if not button_present or button_disabled:
            return
        if any(text in body_text for text in ("发布成功", "审核中", "处理中", "草稿箱")):
            return
        time.sleep(1)

    raise PublishError("点击发布后页面未进入新状态，可能未成功提交")
