...
def run_publish_pipeline(
    ...
    if not ensure_chrome(port=port, headless=headless, user_data_dir=profile_dir):
        return {"success": False, "error": "无法启动 Chrome，请检查安装和 profile 配置"}
        ...
        try:
            ...
            if not check_login_status(page):
                ...
                if headless:
                    ...
                    restart_chrome(port=port, headless=False, user_data_dir=profile_dir)
                    return {
                        "success": False,
                        "error": "未登录",
