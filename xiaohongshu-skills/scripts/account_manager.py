...
_DEFAULT_PROFILE_DIR = _CONFIG_DIR / "chrome-profile"
...
def resolve_account_name(account: str) -> str:
    """解析最终生效的账号名称。"""
    return account.strip() or get_default_account().strip()


def get_profile_dir(account: str = "") -> str:
    """获取账号对应的 Chrome Profile 目录。"""
    resolved = resolve_account_name(account)
    if resolved:
        return _get_profile_dir(resolved)
    return str(_DEFAULT_PROFILE_DIR)
